/* ----------------- Валидация ввода даты заказа ----------------- */
const cleavePhone = new Cleave('#order_date', {
    delimiter: '.',
    blocks: [2, 2, 4],
});

// Собираем все dropdown'ы
const dropdowns_in_filter = document.querySelectorAll('.dropdown.in_filter');

// Пробегаемся по всем dropdown'ам
dropdowns_in_filter.forEach(function(dropdown, drop_index, array) {
...
document.addEventListener('click', function(event) {
    ...
    if (!event.target.closest('.dropdown')) {
        dropdowns_in_filter.forEach(function(element) {
            element.classList.remove('active');
        });
    }
});